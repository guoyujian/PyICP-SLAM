...
def get_frames(pcd_files):
    ...
    for i, pcd_f in enumerate(pcd_files):
        ...
        offset_accumulation += np.linalg.norm(transformation[:3, 3])
...
def loop_and_merge(args):


    pcd_files = glob(osp.join(args.pcd_dir_path, '*.pcd'))
    ...
    o3d.io.write_point_cloud(args.pcd_file_path,
         merged_pcd
    )
